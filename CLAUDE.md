...
- **`scan.py`** — incremental: skips files where `mtime + size_bytes` match the existing DB row. Junk detection mirrors `clean.py` patterns exactly; junk files are never indexed.

### Critical design constraints

- **Every destructive command defaults to dry-run.** `clean` and `dedupe --apply` are the only two commands that modify the filesystem. Both require an explicit `--apply` flag.
...
- **Junk patterns are defined in two places** (`clean.py` and `scan.py`) and must stay in sync. If you add a new junk pattern, add it to both modules.
...
- **Phase 1** ✅ — `sfx` CLI package (clean, scan, dedupe, audit, search, export)
- **Phase 2** 🔜 — bulk rename (`sfx rename`), metadata writing (`sfx tag`), normalization
- **Phase 3** ⬜ — Tauri GUI shell wrapping the CLI
