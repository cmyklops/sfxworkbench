...
def _audit_redundant_nesting(root: Path, depth: int) -> OrganizeAuditReport:
    ...
    for path in dirs:
        ...
        if path_stats["direct_files"] == 0 and path_stats["child_dirs"] == 1 and path_stats["audio_files"] > 0:
            ...
            if not _is_numeric_category_parent(path, only_child):
                add_candidate(
                    path,
...
def audit_organization(root: Path, pattern: str = "strip-leading-numbers", depth: int = 1) -> OrganizeAuditReport:
    ...
    if pattern == "vendor-product-folders":
        return _audit_vendor_product_folders(root, depth)
    if pattern == "common-prefix-folders":
        return _audit_common_prefix_folders(root, depth)
    if pattern == "numeric-series-folders":
        return _audit_numeric_series_folders(root, depth)
