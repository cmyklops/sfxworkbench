...
sfxworkbench can detect UCS-looking names with a heuristic regex and can import the
...
- `sfxworkbench/ucs.py` is the shared home for current UCS stem parsing.
...
initiative and points users to a Dropbox repository for resources. sfxworkbench
...
the catalog without waiting for a sfxworkbench release.

## Integration Plan

Keep expanding the dedicated `sfxworkbench/ucs.py` module so UCS parsing does not
live separately in feature modules.

Implemented API (heuristic, in `sfxworkbench/ucs.py`):

- `looks_ucs(stem: str) -> bool`
- `parse_ucs_stem(stem: str) -> UcsParseResult`

Implemented API (catalog import, in `sfxworkbench/ucs_catalog.py`):
...
- `load_catalog(path=None) -> UcsCatalog | None` — discovery chain:
  explicit path → `SFXWORKBENCH_UCS_DATA` env var → `~/.sfxworkbench/ucs_catalog.json`
  cache → `None`
...
- `default_cache_path() -> Path` — returns `~/.sfxworkbench/ucs_catalog.json`
...
uv run sfx ucs validate --db ~/.sfxworkbench/index.db --json
...
uv run sfx tag suggest PATH --ucs-catalog ~/.sfxworkbench/ucs_catalog.json --output ~/reports/tag_suggestions_ucs.json
```

Future data behavior:

- Support user-supplied UCS CSV/JSON via `--ucs-data` or `SFXWORKBENCH_UCS_DATA`.
- Cache imported catalogs under `~/.sfxworkbench/` or in SQLite.
- Add an importer for the official `UCS v8.2.1 Full List.xlsx` layout.
- Add a normalized `sfxworkbench/data/ucs_categories.json` generated from a pinned
